...
int main(){
    ...
    FILE *arq1;
    arq1 = fopen("iteracoesCasoMedioLinearOrdenado.txt", "w+");
    ...
    for(max = 5000; max <= 50000; max += 5000){
        ...
        for(int j = 0; j < 30; j++){
            ...
            buscaLinearOrdenada(vet, max, pos);
            fprintf(arq1, "%d, ", cont);
            cont = 0;
        }
        fprintf(arq1, "\n");
    ...
    FILE *arq2;
    arq2 = fopen("iteracoesPiorCasoLinearOrdenado.txt", "w+");
    ...
    for(max = 5000; max <= 50000; max += 5000){
        ...
        buscaLinearOrdenada(vet, max, max+1);
        fprintf(arq2, "%d, ", cont);
        cont = 0;
